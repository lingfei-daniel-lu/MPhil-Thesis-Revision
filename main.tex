...
% \author{\large \href{http://yaoli.people.ust.hk/}{Yao Amber Li}\thanks{Li: Department of Economics and Faculty Associate of the Institute for Emerging Market Studies (IEMS), Hong Kong University of Science and Technology,
% Clear Water Bay, Kowloon, Hong Kong SAR-PRC. Email: \href{mailto:yaoli@ust.hk}{yaoli@ust.hk}.}\\ \large{HKUST}
% \and  \large \href{https://sites.google.com/site/sryeaple/}{ Stephen R. Yeaple}\thanks{Yeaple: Department of Economics, the Pennsylvania State University, 520 Kern Building, University Park, PA 16801, USA.  Email: \href{mailto:sry3@psu.edu}{sry3@psu.edu}.  }\\ \large{PSU, NBER \& CESifo } 
% \and \large \href{}{Tengyu Zhao}\thanks{Zhao: Department of Economics, Hong Kong University of Science and Technology, Clear Water Bay, Kowloon, Hong Kong SAR-PRC. Email: \href{mailto:}{tzhaoah@connect.ust.hk}.} \\ \large{HKUST}
% }
...
This paper contributes to three strands of literature. First, this paper contributes to a wide literature on exchange rate disconnect (\cite{obstfeld2000}), particularly on trade pricing response to exchange rate changes (\cite{campa2005}). Recently, more studies link the exchange rate pass-through to disaggregated firm-level characteristics. \cite{bmm2012} find that exporters with better performance react to a exchange rate shock by adjusting more their markup and less their export volume. \cite{aik2014} find that firms with higher import intensity and larger market share have less complete price pass-through. Many follow-up works find that exporters' productivity (\cite{lmx2015}) and product quality (\cite{chen2016};\cite{auer2018}), as well as their imported inputs (\cite{wang-yu2021}), market shares (\cite{auer2016}; \cite{devereux2017}) and information access (\cite{garetto2016}), will all affect the export exchange rate pass-through. 

For Chinese exchange rate pass-through, \cite{lmx2015} find nearly complete exchange rate pass-through into RMB price for Chinese exporters, which means that their domestic currency price response is very weak. Yet the role of importers in determining exchange rate pass-through remains a novel field to study. Our contribution is to provides micro-level evidence of Chinese importers' heterogeneity in response to exchange rate shocks. This provides a novel perspective to study exchange rate disconnect in emerging economies, where firms are financially more vulnerable due to immature financial markets.

Second, this paper relates to the literature on credit constraints and trade (\cite{kroszner2007}, \cite{manova2013}, \cite{chaney2016}, \cite{feenstra2015}, \cite{manova-wei-zhang2015}, \cite{fan-lai-li2015}). We study the influence of credit constraints on firms’ response in price setting to exchange rate fluctuations. The most relevant study to this paper is Strasser (2013), which discusses the relationship between exchange rate pass-through and credit constraints. Strasser (2013) first uses a firm-level survey to show that financially constrained exporters pass exchange rate shocks to prices more completely. Subject to variable markups, firms under borrowing constraints must keep the pricing-to-market (PTM) strategy to a limited extent because they have less space to adjust their profit margin. 

Two recent articles provide Chinese evidence on credit constraints and exchange rate elasticity of trade (\cite{dai2021} and \cite{xu-guo2021}). Both studies verify the conclusion of \cite{strasser2013} that more financially constrained firms' exporting activities are more sensitive to exchange rate changes. \cite{dai2021} focus more on aggregate export behaviors rather than bilateral trade elasticity. \cite{xu-guo2021} imply that variable markups cannot fully explain the effect of credit constraints on exchange rate elasticity. In this paper, we contribute to this literature by focusing on how importers under varying degrees of credit constraints have heterogeneous capacity to absorb exchange rate shocks.

Third, we contribute to the literature on import sourcing (?) 
...
\subsection{Customs Transaction Records}
...
\subsection{Country-level Macro Data}
...
\subsubsection{Estimations with credit constraints}\label{seq-4.1.2}
...
\subsection{Import Pass-through vs Export Pass-through}
...
\subsection{Effects of Credit Constraints}
...
\section{Robustness}\label{sec-6.robustness}
...
\section{More Discussions}\label{sec-7.discussion}
...
\subsection{Firm Heterogeneity in Ownership} \label{sec-7.3}
...
\subsection{Discussion on Import-export Linkage} \label{sec-7.4}
...
\appendix{Extra Tables}\label{sec-appendix}
